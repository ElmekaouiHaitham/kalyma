...
export default function PracticePage() {
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-full text-gray-500">Loading your deck...</div>;
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-full p-6 text-center space-y-8 max-w-lg mx-auto">
        ...
        <div className="space-y-2">
          <h1 className="text-3xl font-bold text-[#1a2b5e] font-outfit">You're all caught up!</h1>
          <p className="text-[#64748b]">There are no cards due for review right now. Add more vocabulary from articles or check back later.</p>
        </div>
        <div className="flex gap-4 items-center mt-4">
          <div className="text-center px-6 py-4 bg-white rounded-2xl shadow-sm border border-[#1a2b5e]/5">
            <div className="text-2xl font-bold text-[#1a2b5e]">{stats.total_saved}</div>
            <div className="text-xs font-bold text-[#9aa5b1] uppercase">Total Saved</div>
          </div>
        </div>
        <button 
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-full p-6 text-center space-y-8 max-w-2xl mx-auto">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 w-full">
          {[
            { label: "Total Saved", val: stats.total_saved.toString(), icon: "📚", color: "text-[#1a2b5e]" },
            { label: "Accuracy", val: deck.length > 0 ? `${Math.round(((scores.good + scores.easy) / deck.length) * 100)}%` : "0%", icon: "🎯", color: "text-green-500" },
            { label: "Good/Easy", val: (scores.good + scores.easy).toString(), icon: "✅", color: "text-blue-500" },
            { label: "Again/Hard", val: (scores.again + scores.hard).toString(), icon: "🔥", color: "text-orange-500" },
          ].map((stat, i) => (
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
             <div className="text-right">
                <div className="text-sm font-bold text-[#1a2b5e]">{currentIndex + 1} / {deck.length}</div>
      ...
      <div className="flex-1 w-full flex items-center justify-center p-2 sm:p-4">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="w-full max-w-md min-h-[400px] max-h-[65vh] bg-white rounded-[2rem] sm:rounded-[3rem] p-6 sm:p-10 flex flex-col shadow-xl shadow-[#1a2b5e]/5 border border-[#1a2b5e]/10 relative overflow-y-auto cursor-pointer group hover:border-[#1a2b5e]/20 transition-all custom-scrollbar"
            onClick={() => !isFlipped && setIsFlipped(true)}
          >
            {/* Background Accent */}
            <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-[#1a2b5e] to-[#c9a84c] opacity-80 shrink-0" />

            <div className="m-auto w-full flex flex-col items-center justify-center text-center space-y-6 sm:space-y-8">
              {!isFlipped ? (
                <>
                  <div className="space-y-4">
                    ...
                    <h2 className={`font-black text-[#1a2b5e] font-outfit leading-tight break-words ${currentCard.item.content.length > 50 ? 'text-2xl sm:text-3xl' : 'text-3xl sm:text-4xl'}`}>
                ...
                <div className="space-y-6 sm:space-y-8 w-full">
                  <div className="space-y-4">
                     ...
                     <h3 className={`font-bold text-[#1a2b5e] leading-snug ${currentCard.item.translation?.length > 100 ? 'text-xl sm:text-2xl' : 'text-2xl sm:text-3xl'}`}>
                  ...
                  {currentCard.item.context && (
                    <div className="bg-[#f8faff] p-5 sm:p-7 rounded-2xl sm:rounded-3xl border border-[#1a2b5e]/5 relative">
                       <div className="absolute -top-3 left-6 px-3 py-1 bg-white border border-[#1a2b5e]/5 rounded-lg text-[10px] font-bold text-[#1a2b5e] uppercase tracking-wider">Context</div>
      ...
      <div className="w-full shrink-0 min-h-[120px] flex items-center justify-center">
        <AnimatePresence mode="wait">
          {!isFlipped ? (
            ...
            >
              {[
                { label: "Again", type: "again", rating: 0, color: "from-[#ff6b6b] to-[#ee5253]", time: " " },
                { label: "Hard", type: "hard", rating: 1, color: "from-[#feca57] to-[#ff9f43]", time: " " },
                { label: "Good", type: "good", rating: 3, color: "from-[#48dbfb] to-[#0abde3]", time: " " },
                { label: "Easy", type: "easy", rating: 5, color: "from-[#1dd1a1] to-[#10ac84]", time: " " },
              ].map((btn) => (
                <button
                  key={btn.type}
